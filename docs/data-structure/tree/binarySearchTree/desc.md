...
注意，第三条结论也比较重要，因为我们可以用这条性质来判断一个树是否是二叉搜索树。
...
这儿有一个需要注意的店是假设我们删除的时候以当前节点的左子树最大值替换待删除节点的值，一定不要忘了，**左子树的最大值是有可能有左子树的，一定不要忘了把这个子树给拼接上，否则就会丢失这一大块存储区域。**
...
二叉搜索树理想情况插入、查找、删除的效率是非常高的，是典型的二分查找思想，能达到 `O(logN)`，但是理想很丰满，现实很骨感。假设我们**插入有序序列**或者**总是删除二叉搜索树的最大值或者最小值**，最终我们的**二叉搜索树竟然会成为一个链表**，效率直接降到 `O(N)`，所以如何在插入或删除的时候总是使得我们的树左半部分和右半部分节点数差异不大，那么我们查找和删除的效率总能达到 `O(logN)`，这便是 `AVL 树`或`红黑树`。`AVL 树`或`红黑树`在后面我们会详细讨论。
