...
export function combinationSum(
  ...
  const calcCombination = (
    ...
    if (offset >= options.length || options[offset] > sumTarget) {
      return;
    }
    if (options[offset] === sumTarget) {
      res.push([...prevRes, options[offset]]);
      return;
    }
    // 不选
    calcCombination(options, offset + 1, sumTarget, prevRes);
    // 选
    calcCombination(options, offset, sumTarget - options[offset], [
      ...prevRes,
      options[offset],
    ]);
  };

  calcCombination(candidates, 0, target, []);
  return res;
}
